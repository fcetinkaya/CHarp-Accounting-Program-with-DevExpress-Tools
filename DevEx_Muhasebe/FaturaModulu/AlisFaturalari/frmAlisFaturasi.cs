...
namespace Gelisim_Muhasebe.FaturaModulu.AlisFaturalari
{
    ...
    public partial class frmAlisFaturasi : DevExpress.XtraEditors.XtraForm
    {
        ...
        cls_AlisFaturasi S = new cls_AlisFaturasi();
        ...
        private void frmSatisFaturasi_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    txtFtNo.Text = N.GetYeniAlisFtNumarasi();
                }
            }
            catch (Exception ex)
            {
        ...
        public void FtKayit()
        {
            ...
            if (!S.FtNumarasiKontrol(txtFtNo.Text))
            {
                int aydiGel = Convert.ToInt32(S.faturaKayit(txtFtNo.Text, txtBelgeNumarasi.Text, cariKodBtn.Text, txtTarih.Text, txtVadeTarihi.Text, comboFtTuru.SelectedItem.ToString(), "Alış Faturası", 1, txtAciklama.Text, Convert.ToDouble(txtGenelToplam.Text), CariId, frmAnamenu.KullaniciID.ToString(), FSayi, DBNull.Value.ToString(), DBNull.Value.ToString()));
                if (aydiGel > 0)
                {
                    FtUrunKalemleriKayit(aydiGel.ToString());
                    S.CariHareketKayit(txtFtNo.Text, txtBelgeNumarasi.Text, txtTarih.Text, 0, Convert.ToDouble(txtGenelToplam.Text), txtAciklama.Text, txtVadeTarihi.Text, "Alış Faturası", "1", CariId, aydiGel.ToString(), frmAnamenu.KullaniciID.ToString());

                    if (comboFtTuru.SelectedIndex == 1)
                    {
                        S.CariHareketKayit(txtFtNo.Text, txtBelgeNumarasi.Text, txtTarih.Text, Convert.ToDouble(txtGenelToplam.Text), 0, txtAciklama.Text, txtVadeTarihi.Text, "Alış Faturası", "1", CariId, aydiGel.ToString(), frmAnamenu.KullaniciID.ToString());
        ...
        public void FtGuncelle()
        {
            ...
            if (S.faturaGuncelle(txtFtNo.Text, txtBelgeNumarasi.Text, cariKodBtn.Text, txtTarih.Text, txtVadeTarihi.Text, comboFtTuru.SelectedItem.ToString(), txtAciklama.Text, Convert.ToDouble(txtGenelToplam.Text), CariId, FSayi, frmAnamenu.KullaniciID.ToString(), _ftID))
            {
                ...
                S.CariHareketKayit(txtFtNo.Text, txtBelgeNumarasi.Text, txtTarih.Text, 0,Convert.ToDouble(txtGenelToplam.Text), txtAciklama.Text, txtVadeTarihi.Text, "Alış Faturası", "1", CariId, _ftID, frmAnamenu.KullaniciID.ToString());

                if (comboFtTuru.SelectedIndex == 1)
                {
                    S.CariHareketKayit(txtFtNo.Text, txtBelgeNumarasi.Text, txtTarih.Text, Convert.ToDouble(txtGenelToplam.Text), 0,txtAciklama.Text, txtVadeTarihi.Text, "Alış Faturası", "1", CariId, _ftID, frmAnamenu.KullaniciID.ToString());
                    if (OdemeturuCombo.SelectedIndex == 0)
                    {
        ...
        public void FtUrunKalemleriKayit(string gelenFtID)
        {
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                ...
                S.stokHareketKayit(txtFtNo.Text, txtBelgeNumarasi.Text, "G", _gelenMiktar, _gelenFiyat, _gelenIsk1Oran, _gelenIsk2Oran, _gelenKdvOrani, txtAciklama.Text, "Alış Faturası", txtTarih.Text, frmAnamenu.SecilenCariID, _GelenStokID, gelenFtID, frmAnamenu.KullaniciID.ToString());
            }
        }
        ...
        private void SatisFtListesiSecBtn_Click(object sender, EventArgs e)
        {
            try
            {
                frmAlisFaturasiListesi SF = new frmAlisFaturasiListesi();
                SF.ShowDialog();
                _ftID = frmAnamenu.SecilenFtID;
                string[] x = S.FtBilgileri_Gel(_ftID);
                if (!string.IsNullOrEmpty(x[0]))
